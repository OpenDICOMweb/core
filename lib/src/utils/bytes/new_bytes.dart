...
class Bytes extends ListBase<int> {
  ...
  bool operator ==(Object other) {
    if (other is _Bytes) {
      if (bb.lengthInBytes != other.lengthInBytes) return false;
      for (var i = 0; i < length; i++)
        if (bb[].getUint8(i) != other._bd.getUint8(i)) return false;
      return true;
    }
    return false;
  }
}
abstract class _LEBytes extends _Bytes {
...
/// [_Bytes] is a class that provides a read-only byte array that supports both
/// [Uint8List] and [ByteData] interfaces.
abstract class _Bytes extends {
  ByteBuffer  get bb;
  int get _off_len;
  Endian get endian;

  static int _toData(int offset, int length) => (offset << 32) + (length & 0xFFFFFFFF);

  /// Returns a
//h  factory _Bytes([int length = kDefaultLength, Endian endian = Endian.little]) {}

//  _Bytes._ofLength(this._bb, this._off_len);


/*
  _Bytes.from(_Bytes bytes,
      [int offset = 0, int length, this.endian = Endian.little])
      : _bd = bytes.asByteData(offset, length ?? bytes.lengthInBytes);

  _Bytes.fromList(List<int> list, [this.endian = Endian.little])
      : _bd = (list is Uint8List)
            ? list.buffer.asByteData()
            : (new Uint8List.fromList(list)).buffer.asByteData();

  _Bytes.typedDataView(TypedData td, [this.endian = Endian.little])
      : _bd = td.buffer.asByteData();

  _Bytes.view(_Bytes bytes,
      [int offset = 0, int length, this.endian = Endian.little])
      : _bd = bytes.asByteData(offset, length ?? bytes.lengthInBytes);


  _Bytes.typedDataView(TypedData td,
      [int offset = 0, int length, this.endian = Endian.little])
      : _bd = td.buffer.asByteData(
            td.offsetInBytes + offset, length * td.elementSizeInBytes);

  _Bytes._(this._bb) : off_leng = _bb.lengthInBytes
*/

  int get offset => _off_len >> 32;
  int get length => _off_len & 0xFFFFFFFF;


  // Core accessor NOT to be exported?
//  ByteData get bd => new ByteData.;
  ...
  @override
  _Bytes sublist(int start, [int end]) =>
      new _Bytes.from(this, start, end ?? length, endian);
  ...
  _Bytes subbytes([int start = 0, int end]) =>
      new _Bytes.view(this, start, end ?? length);

  // Returns a view of _this_.
  _Bytes asBytes([int offset = 0, int length]) =>
      new _Bytes.view(this, offset, length ?? lengthInBytes);
  ...
  Uint64List asUint64List([int offset = 0, int length]) {
    ...
    /// Returns a [_Bytes] buffer containing the contents of [File].
    // TODO: add async
    // TODO: unit test
    static _Bytes fromFile(File file,
        {Endian endian = Endian.little, bool doAsync = false}) {
      final Uint8List iList = file.readAsBytesSync();
      return new _Bytes.typedDataView(iList, endian);
    }

    /// Returns a [_Bytes] buffer containing the contents of the
    /// [File] at [path].
    // TODO: add async
    // TODO: unit test
    static _Bytes fromPath(String path,
        {Endian endian = Endian.little, bool doAsync = false}) =>
        fromFile(new File(path), endian: endian, doAsync: doAsync);

    static final _Bytes kEmptyBytes = new _Bytes._();


    static _Bytes base64Decode(String s) =>
    new _Bytes.typedDataView(base64.decode(s));

    static String base64Encode(_Bytes bytes) => base64.encode(bytes.asUint8List());

    static String asciiDecode(_Bytes bytes, {bool allowInvalid = true}) =>
        ascii.decode(bytes.asUint8List(), allowInvalid: allowInvalid);

    static _Bytes asciiEncode(String s) =>
    new _Bytes.typedDataView(ascii.encode(s));

    static String utf8Decode(_Bytes bytes, {bool allowMalformed = true}) =>
        utf8.decode(bytes.asUint8List(), allowMalformed: allowMalformed);

    static _Bytes utf8Encode(String s) => new _Bytes.typedDataView(ascii.encode(s));
}

class GrowableBytes extends _Bytes {
  /// The upper bound on the length of this [_Bytes]. If [limit]
  /// is _null_ then its length cannot be changed.
  final int limit;

  /// Returns a new [_Bytes] of [length].
  GrowableBytes(int length,
      [Endian endian = Endian.little, this.limit = kDefaultLimit])
      : super(length, endian);

  /// Returns a new [_Bytes] starting at [offset] of [length].
