...
*/
  ...
  int getInt8() => buffer.getInt8(rIndex_);

  int readInt8() {
    final v = buffer.getInt8(rIndex_);
    rIndex_++;
    return v;
  }

  int getInt16() => buffer.getInt16(rIndex_);

  int readInt16() {
    final v = buffer.getInt16(rIndex_);
    rIndex_ += 2;
    return v;
  }

  int getInt32() => buffer.getInt32(rIndex_);

  int readInt32() {
    final v = buffer.getInt32(rIndex_);
    rIndex_ += 4;
    return v;
  }

  int getInt64() => buffer.getInt64(rIndex_);

  int readInt64() {
    final v = buffer.getInt64(rIndex_);
    rIndex_ += 8;
    return v;
  }

  int getUint8() => buffer.getUint8(rIndex_);

  int readUint8() {
    final v = buffer.getUint8(rIndex_);
    rIndex_++;
    return v;
  }

  int getUint16() => buffer.getUint16(rIndex_);

  int readUint16() {
    final v = buffer.getUint16(rIndex_);
    rIndex_ += 2;
    return v;
  }

  int getUint32() => buffer.getUint32(rIndex_);

  int readUint32() {
    final v = buffer.getUint32(rIndex_);
    rIndex_ += 4;
    return v;
  }

  int getUint64() => buffer.getUint64(rIndex_);

  int readUint64() {
    final v = buffer.getUint64(rIndex_);
    rIndex_ += 8;
    return v;
  }

  String readAscii(int length) {
    final s = buffer.getAscii(rIndex_, length);
    rIndex_ += length;
    return s;
  }
  String readUtf8(int length) {
    final s = buffer.getUtf8(rIndex_, length);
    rIndex_ += length;
    return s;
  }
  ...
  int peekCode() {
    ...
    final group = buffer.getUint16(rIndex_);
    final elt = buffer.getUint16(rIndex_ + 2);
    return (group << 16) + elt;
  }
  ...
  bool getUint32AndCompare(int target) {
    final delimiter = buffer.getUint32(rIndex_);
  ...
  Uint8List uint8View([int start = 0, int length]) {
    final offset = _getOffset(start, length);
    return buffer.asUint8List(offset, length ?? lengthInBytes - offset);
  }

  Uint8List readUint8View(int length) => uint8View(rIndex_, length);

  Int8List readInt8List(int length) {
    final v = buffer.getInt8List(rIndex_, length);
    rIndex_ += length;
    return v;
  }

  Int16List readInt16List(int length) {
    final v = buffer.getInt16List(rIndex_, length);
    rIndex_ += length;
    return v;
  }

  Int32List readInt32List(int length) {
    final v = buffer.getInt32List(rIndex_, length);
    rIndex_ += length;
    return v;
  }

  Int64List readInt64List(int length) {
    final v = buffer.getInt64List(rIndex_, length);
    rIndex_ += length;
    return v;
  }

  Uint8List readUint8List(int length) {
    final v = buffer.getUint8List(rIndex_, length);
    rIndex_ += length;
    return v;
  }

  Uint16List readUint16List(int length) {
    final v = buffer.getUint16List(rIndex_, length);
    rIndex_ += length;
    return v;
  }

  Uint32List readUint32List(int length) {
    final v = buffer.getUint32List(rIndex_, length);
    rIndex_ += length;
    return v;
  }

  Uint64List readUint64List(int length) {
    final v = buffer.getUint64List(rIndex_, length);
    rIndex_ += length;
    return v;
  }

  Float32List readFloat32List(int length) {
    final v = buffer.getFloat32List(rIndex_, length);
    rIndex_ += length;
    return v;
  }

  Float64List readFloat64List(int length) {
    final v = buffer.getFloat64List(rIndex_, length);
    rIndex_ += length;
    return v;
  }

  List<String> readAsciiList(int length) {
    final v = buffer.asAsciiList(rIndex_, length);
    rIndex_ += length;
    return v;
  }

  List<String> readUtf8List(int length) {
    final v = buffer.asUtf8List(rIndex_, length);
    rIndex_ += length;
    return v;
  }
  ...
  int _getOffset(int start, int length) {
    final offset = buffer.offsetInBytes + start;
  ...
  ByteData toByteData(int offset, int lengthInBytes) =>
      buffer.buffer.asByteData(buffer.offsetInBytes + offset, lengthInBytes);
