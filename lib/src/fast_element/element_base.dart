...
abstract class FL extends Float with Float32Mixin {
  ...
  static int checkVRIndex(int vrIndex) =>
      (vrIndex == kVRIndex) ? vrIndex : badVRIndex(vrIndex, kVRIndex);
  ...
  static bool isValidValue(double value) => true;
}
...
//  static const VR kVR = VR.kFD;
  ...
  static bool isValidArgs(Tag tag, Iterable<double> vList) {
    ...
    if (doTestValidity) return isValidValues(tag, vList);
    return true;
  }
...
//  static const VR kVR = VR.kOD;
  ...
  static bool isValidLength(int length) => true;
