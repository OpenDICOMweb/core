...
import 'package:core/src/element/base/meta.dart';
...
class DatasetComparitor<K> {
  ...
  DatasetComparitor(this.dataset0, this.dataset1, [Level logLevel = Level.config]) {
    log.level = logLevel;
  }
  ...
  bool compare(Dataset ds0, Dataset ds1) {
    ...
    log..debug('ds0 length(${ds0.length}', 1)..debug('ds1 length(${ds1.length}');
    length = ds0.length;
