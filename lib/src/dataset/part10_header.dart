...
class Part10Header<K, V> {
  ...
  Part10Header(this.bd, this.fmi, this._ts) {
    if (!hasPrefix) throw new ArgumentError('Invalid Prefix: "$prefix"');
    if (fmi.isEmpty) throw new ArgumentError('Empty FMI: $fmi');
  }
  ...
  TransferSyntax get ts => _ts ?? system.defaultTransferSyntax;
  ...
  bool get wasPreambleZeros => (hasPrefix) ? _checkForZeros() : false;
  ...
  static bool checkPrefix(ByteData bd) {
    ...
    final s = ASCII.decode(bd.buffer.asUint8List(128, 4));
    return (s == prefix);
  }
}
