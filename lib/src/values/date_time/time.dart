...
typedef OnTimeError = Time Function(
    int h, int m, int s, int ms, int us, Exception error);
typedef OnTimeParseError = Time Function(String s);
typedef OnTimeHashStringError = String Function(String s);
...
class Time implements Comparable<Time> {
  ...
  factory Time(int h,
    ...
    try {
      ...
      return (uSecs == null) ? null : Time.fromMicroseconds(uSecs);
  ...
  Time operator +(Time other) =>
      Time.fromMicroseconds(uSeconds + other.uSeconds);
  ...
  /// Returns a [Time] containing the [hash] of _this_.
  Time get hash => Time.fromMicroseconds(hashTimeMicroseconds(uSeconds));

  /// Returns a [Time] containing the SHA-256 hash of [uSeconds].
  Time get sha256 => Time.fromMicroseconds(sha256Microseconds(uSeconds));
  ...
  Duration get asDuration => Duration(microseconds: uSeconds);
  ...
  String timeToString({bool asDicom = true}) {
    final sb = StringBuffer()..write(digits2(hour));
    if (!asDicom) sb.write(':');
