...
typedef Element ElementFromBytes(Bytes bytes, int vrIndex);
...
/*
  Element makeElement(int code, Bytes bytes, int vrIndex);

  SQ makeSequence(int code, Dataset parent, List<Item> items,
      Bytes bytes, int vrIndex);
  
  Element makePixelData(int code, Bytes bytes,
      TransferSyntax ts, VFFragments fragments, int vrIndex);
*/
...
abstract class Common {
  ...
  int get code {
    final group = bd.getUint16(_codeOffset);
    final elt = bd.getUint16(_codeOffset + 2);
    final v = (group << 16) + elt;
    return v;
  }

  int get group => bd.getUint16(_groupOffset);
  int get elt => bd.getUint16(_eltOffset);
...
//  int get deIdIndex => unimplementedError();
  ...
  Bytes get vfBytesWithPadding => (bd.lengthInBytes == vfOffset)
      ? kEmptyBytes
      : bd.asBytes(bd.offsetInBytes + vfOffset, vfLength);

  Bytes get vfBytes => (bd.lengthInBytes == vfOffset)
      ? kEmptyBytes
      : bd.asBytes(bd.offsetInBytes + vfOffset, vfLength);
}
...
bool bytesEqual(Bytes bytes0, Bytes bytes1) =>
    byteDataEqual(bytes0.asBytes(), bytes0.asBytes());

/// Returns _true_ if all bytes in [bd0] and [bd1] are the same.
/// _Note_: This assumes the [Bytes] is aligned on a 2 byte boundary.
bool byteDataEqual(Bytes bd0, Bytes bd1, {bool doFast = false}) {
...
/// Returns _true_ if all bytes in [bytes0] and [bytes1] are the same.
/// _Note_: This assumes the [Bytes] is aligned on a 2 byte boundary.
bool bytesListEqual(Bytes bytes0, Bytes bytes1) {
  final bd0 =
  bytes0.asBytes(bytes0.offsetInBytes, bytes0.lengthInBytes);
  final bd1 =
  bytes1.asBytes(bytes1.offsetInBytes, bytes1.lengthInBytes);
...
bool uint8ListEqual(Uint8List bList0, Uint8List bList1) {
  if (bList0.lengthInBytes != bList1.lengthInBytes) return false;
  for (var i = 0; i < bList0.lengthInBytes; i++)
    if (bList0[i] != bList1[i]) return false;
  return true;
}

bool bytesEqualSlow(Bytes bd0, Bytes bd1) {
  var ok = true;
...
bool bytesEqualFast(Bytes bd0, Bytes bd1) {
  var ok = true;
...
void _toBytes(int i, Bytes bd0, Bytes bd1) {
  final bytes0 = bd0.asBytes(i, 8);
  log.warn('    $bytes0');
  final bytes1 = bd1.asBytes(i, 8);
  log.warn('    $bytes1');
...
/// Returns _true_ if all bytes in [bd0] and [bd1] are the same.
/// _Note_: This assumes the [Bytes] is aligned on a 2 byte boundary.
bool bdEqual(Bytes bd0, Bytes bd1) => byteDataEqual(bd0, bd1);

int getLength(Bytes vfBytes, int unitSize) {
...
bool checkPadding(Bytes bd, [int padChar = kSpace]) {
...
//TODO: This should be done in convert
Bytes removePadding(Bytes bd, int vfOffset, [int padChar = kSpace]) {
  ...
  if (char == kNull || char == kSpace) {
    ...
    return bd.asBytes(bd.offsetInBytes, bd.lengthInBytes - 1);
  }
  return bd;
}
