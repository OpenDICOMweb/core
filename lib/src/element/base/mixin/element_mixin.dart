...
/// is the [Type] of the [values] of the [ElementMixin].
abstract class ElementMixin<V> {
  // **** Interface
  ...
  ElementMixin<V> update([Iterable<V> vList]);
  // **** end Interface
  ...
  /// Returns _true_ if this is a _Group Length_ [ElementMixin].
  /// Group Length [ElementMixin]s have codes in the format (gggg,0000).
  /// _Note_: Group Length [ElementMixin]s are _retired_, but they appear
  /// in many existing DICOM Datasets.
  bool get isGroupLength => elt == 0;

  /// Returns true if _this_ is Data [ElementMixin] defined by an implementation.
...
//  Condition get eTypePredicate => unimplementedError();
  ...
  /// _true_ if this [ElementMixin] had an undefined length token in the
...
/*
  bool valuesEqual(ElementMixin<V> other) =>
      vListEqual<V>(values, other.values);
*/
...
//  bool get hasValidVR => tag.vrIndex == vrIndex;
  ...
  ElementMixin get sha256 => throw new UnimplementedError();

  /// Returns a copy of _this_, but with [values] replaced by
  /// a [List] where each [value] has been replaced with an
  /// AES-DCM encryption of the initial [value].
  ElementMixin get encrypted => throw new UnimplementedError();

  /// Returns a copy of _this_, but with [values] replaced by
  /// a [List] where each [value] has been replaced with a
  /// AES-DCM decryption of the initial encrypted [value].
  ElementMixin get decrypted => throw new UnimplementedError();

  /// Returns the total number of [ElementMixin]s in _this_.
...
//  Element updateF(Iterable<V> f(Iterable<V> vList)) => update(f(values));
  ...
  /// [values]. This method modifies the [ElementMixin].
  ...
  /// and return the original [values]. This method modifies the [ElementMixin].
  Iterable<V> replaceF(Iterable<V> f(Iterable<V> vList)) => replace(f(values));

  /// Returns a copy of _this_.
  ElementMixin<V> get copy => update(valuesCopy);

  int counter(bool test(ElementMixin e)) => test(this) ? 1 : 0;
...
/*  // ***************** Static Getters and Methods *****************
  // **************************************************************
  static bool equal<V>(ElementMixin<V> a, ElementMixin<V> b) =>
