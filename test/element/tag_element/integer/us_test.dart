...
void main() {
  Server.initialize(name: 'element/us_test', level: Level.debug);
  final rng = new RNG(1);
  ...
  group('US', () {
    test('US hasValidValues good values random', () {
      ...
      for (var i = 0; i < 10; i++) {
        ...
        final e0 =
        new UStag(PTag.kTopLeftHandCornerOfLocalizerArea, vList0);
        expect(e0.hasValidValues, true);
        log.debug('e0: $e0');
        expect(e0.hasValidValues, true);
    ...
    test('US hashCode and == bad values random', () {
      ...
      for (var i = 0; i < 10; i++) {
        ...
        final e3 =
        new UStag(PTag.kTopLeftHandCornerOfLocalizerArea, vList2);
    ...
    test('US fromBytes random', () {
      for (var i = 0; i < 10; i++) {
        ...
        final e1 = UStag.fromBytes(PTag.kRepresentativeFrameNumber, vList1);
        expect(e0 == e1, true);
    ...
    test('US fromBytes', () {
      ...
      final e5 = UStag.fromBytes(PTag.kRepresentativeFrameNumber, bytes0);
      expect(e5.hasValidValues, true);
      expect(e5.vfBytes, equals(bytes0));
      expect(e5.values is Uint16List, true);
      expect(e5.values, equals(vList1));
    });
...
/* Urgent Sharath: DO you still think we need this?
    test('US BASE64 random', () {
      for (var i = 0; i < 10; i++) {
        ...
        final e0 = UStag.fromBytes(PTag.kRepresentativeFrameNumber, base64);
        expect(e0.hasValidValues, true);
      }
    });
...
*/
    ...
    test('US make good values', () {
      for (var i = 0; i < 10; i++) {
        final vList0 = rng.uint16List(1, 1);
        final make0 =
        UStag.fromValues(PTag.kRepresentativeFrameNumber, vList0);
...
*/
    ...
    test('US isValidValues good values', () {
      ...
      //VM.k3
      expect(US.isValidValues(PTag.kTextColorCIELabValue, uInt16MinMaxPle0),
          true);

      //VM.k4
      expect(
          US.isValidValues(PTag.kAcquisitionMatrix, uInt16MinMaxPle1), true);
    ...
    test('US toBytes', () {
      ...
      for (var i = 0; i < 10; i++) {
        ...
        log
          ..debug('vList1 : $vList1')
          ..debug('Uint16Base.listToBytes(vList1) ; '
              '${Uint16.toBytes(vList1)}');
        expect(Uint16.toBytes(vList1), equals(bd));
      }
    ...
    test('Uint16Base listToByteData bad values', () {
      for (var i = 0; i < 10; i++) {
        ...
        final lBd4 =
        Uint16.toByteData(vList0, asView: false, check: false);
...
//        final base64 = cvt.base64.encode(bytes);
        ...
        log.debug('US.base64: $e0');
        final e1 = Uint16.fromBase64(s);
        log.debug('  US.json: $e1');
