...
void main() {
  Server.initialize(name: 'element/ol_test', level: Level.debug);
  final rng = new RNG(1);
...
//        final bd = vList1.buffer.asUint8List();
        ...
        log
          ..debug('vList0 : $vList0')
          ..debug('Uint32Base.toBytes(vList0): '
              '${Uint32.toBytes(vList0)}');
...
//       final base64 = cvt.base64.encode(bd);
        ...
        expect(olList, equals(vList0));
        expect(olList, equals(vList1));
      }
    });
*/ /*
...
//        final s = cvt.base64.encode(bd);
        final bytes = new Bytes.typedDataView(vList0);
        expect(Uint32.toBase64(vList0), equals(s));
      }
    });
*/ /*
...
//       final base64 = cvt.base64.encode(bd);
        ...
        log.debug('OL.base64: $e0');
        final e1 = Uint32.fromBase64(s);
        log.debug('  OL.json: $e1');
