...
void main() {
  ...
  group('UItag', () {
    ...
    test('UI fromValueField', () {
      ...
      for (var i = 1; i < 10; i++) {
        global.throwOnError = false;
        final vList1 = rsg.getUIList(1, i);
      ...
      global.throwOnError = false;
      final vList2 = rsg.getUIList(1, 1);
    ...
    test('UI check', () {
      for (var i = 0; i < 10; i++) {
        ...
        final e0 = UItag(PTag.kSOPClassUID, vList);
        log.debug('e0: $e0');
        expect(e0.hasValidValues, true);
      ...
      for (var i = 1; i < 10; i++) {
        final vList1 = rsg.getUIList(1, i);
        final e0 = UItag(PTag.kSelectorUIValue, vList1);
        expect(e0.hasValidValues, true);
        expect(e0.check(), true);
    ...
    test('UI valuesEqual good values', () {
      for (var i = 1; i < 10; i++) {
        final vList = rsg.getUIList(1, 1);
        final e0 = UItag(PTag.kSelectorUIValue, vList);
        final e1 = UItag(PTag.kSelectorUIValue, vList);
        log.debug('e0: $e0 , e1: $e1');
        expect(e0.valuesEqual(e1), true);
      }
    });

    test('UI valuesEqual bad values', () {
      for (var i = 1; i < 10; i++) {
        final vList0 = rsg.getUIList(1, i);
        final vList1 = rsg.getUIList(1, 1);
        final e0 = UItag(PTag.kSelectorUIValue, vList0);
        final e1 = UItag(PTag.kSelectorUIValue, vList1);
        log.debug('e0: $e0 , e1: $e1');
  ...
  group('UI', () {
    ...
    test('UI fromBytes', () {
      ...
      log.debug('bytes.stringListFromAsciiList(): '
          '${bytes.getAsciiList()}, bytes: $bytes');
      expect(bytes.getAsciiList(), equals(vList1));
    });

    test('UI Bytes.fromAsciiList', () {
      final vList1 = rsg.getUIList(1, 1);
      log.debug('BytesDicom.fromAsciiList(vList1):'
          ' ${BytesDicom.fromAsciiList(vList1)}');
      final val = cvt.ascii.encode('s6V&:;s%?Q1g5v');
      expect(BytesDicom.fromAsciiList(['s6V&:;s%?Q1g5v']), equals(val));

      if (vList1[0].length.isOdd) vList1[0] = '${vList1[0]} ';
      log.debug('vList1:"$vList1"');
      final values = cvt.ascii.encode(vList1[0]);
      expect(BytesDicom.fromAsciiList(vList1), equals(values));
    });
