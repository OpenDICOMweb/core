  ...
  group('LO', () {
    ...
    test('LO isValidVListLength VM.k1 good values', () {
      ...
      for (var i = 0; i < 10; i++) {
        ...
        for (var tag in loTags0) {
          expect(LO.isValidVListLength(tag, vList), true);
    ...
    test('LO isValidVListLength VM.k1 bad values', () {
      for (var i = 1; i < 10; i++) {
        ...
        for (var tag in loTags0) {
          global.throwOnError = false;
          expect(LO.isValidVListLength(tag, vList), false);

          expect(LO.isValidVListLength(tag, invalidVList), false);

          global.throwOnError = true;
          expect(() => LO.isValidVListLength(tag, vList),
              throwsA(const isInstanceOf<InvalidValuesError>()));
        }
      }
    });
    ...
    test('LO isValidVListLength VM.k1_n good values', () {
      ...
      for (var i = 1; i < 10; i++) {
        ...
        for (var tag in loTags1) {
          log.debug('tag: $tag');
          expect(LO.isValidVListLength(tag, vList0), true);
