...
void main() {
  ...
  group('LTtag', () {
    ...
    test('LT hashCode and == good values random', () {
      ...
      for (var i = 0; i < 10; i++) {
        vList = rsg.getLTList(1, 1);
        final e0 = new LTtag(PTag.kImageComments, vList);
        final e1 = new LTtag(PTag.kImageComments, vList);
        log
    ...
    test('LT hashCode and == bad values random', () {
      ...
      for (var i = 0; i < 10; i++) {
        ...
        final e0 = new LTtag(PTag.kImageComments, vList0);
        final vList1 = rsg.getLTList(1, 1);
        final e2 = new LTtag(PTag.kFrameComments, vList1);
...
*/
    ...
    test('LT isValidVListLength VM.k1 good values', () {
      ...
      for (var i = 0; i < 10; i++) {
        ...
        for (var tag in ltTags0) {
          expect(LT.isValidVListLength(tag, vList), true);
    ...
    test('LT isValidVListLength VM.k1 bad values', () {
      for (var i = 1; i < 10; i++) {
        final vList = rsg.getLTList(2, i + 1);
        for (var tag in ltTags0) {
          global.throwOnError = false;
          expect(LT.isValidVListLength(tag, vList), false);

          expect(LT.isValidVListLength(tag, invalidVList), false);

          global.throwOnError = true;
          expect(() => LT.isValidVListLength(tag, vList),
              throwsA(const isInstanceOf<InvalidValuesError>()));
        }
      }
    });
