  ...
  group('UC', () {
    ...
    test('UC isValidVListLength VM.k1 good values', () {
      ...
      for (var i = 0; i < 10; i++) {
        ...
        for (var tag in ucTags0) {
          expect(UC.isValidVListLength(tag, vList), true);
    ...
    test('UC isValidVListLength VM.k1 bad values', () {
      for (var i = 1; i < 10; i++) {
        ...
        for (var tag in ucTags0) {
          global.throwOnError = false;
          expect(UC.isValidVListLength(tag, vList), false);

          expect(UC.isValidVListLength(tag, invalidVList), false);

          global.throwOnError = true;
          expect(() => UC.isValidVListLength(tag, vList),
              throwsA(const isInstanceOf<InvalidValuesError>()));
        }
      }
    });
    ...
    test('UC isValidVListLength VM.k1_n good values', () {
      ...
      for (var i = 1; i < 10; i++) {
        ...
        for (var tag in ucTags1) {
          log.debug('tag: $tag');
          expect(UC.isValidVListLength(tag, vList0), true);
        }
      }
    });
