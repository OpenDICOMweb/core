...
void main() {
  ...
  group('DS Tests', () {
    ...
    test('DS compareTo & compareValueTo', () {
      ...
      for (final vList1 in goodDSList) {
      ...
      for (final n in goodDecimalStrings) {
  ...
  group('DS Element', () {
    ...
    test('DS isValidValueLength bad values', () {
      global.throwOnError = false;
      for (final s in badDSLengthValues) {
        for (final a in s) {
          log.debug(a);
    ...
    test('DS isValidValues bad values length', () {
      global.throwOnError = false;
      for (final s in badDSLengthList) {
        global.throwOnError = false;
    ...
    test('DS toBytes', () {
      ...
      final v = vList[0];
      log.debug('vList0:"$v"');
      final bytes0 = BytesDicom.fromAsciiList(vList);
    ...
    test('DS decodeBinaryStringVF', () {
      ...
      final bytes = BytesDicom.fromAsciiList([]);
    ...
    test('DS toBytes', () {
      for (var i = 0; i < 10; i++) {
        ...
        global.throwOnError = false;
        final toB0 = BytesDicom.fromAsciiList(vList0);
      ...
      for (final s in goodDSList) {
        final toB1 = BytesDicom.fromAsciiList(s);
      ...
      global.throwOnError = false;
      final toB2 = BytesDicom.fromAsciiList(['']);
      expect(toB2, equals(<String>[]));

      final toB3 = BytesDicom.fromAsciiList([]);
      expect(toB3, equals(<String>[]));

      final toB4 = BytesDicom.fromAsciiList(null);
      expect(toB4, isNull);

      global.throwOnError = true;
      expect(() => BytesDicom.fromAsciiList(null),
          throwsA(const TypeMatcher<GeneralError>()));
    });
