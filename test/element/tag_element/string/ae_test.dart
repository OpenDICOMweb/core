...
void main() {
  Server.initialize(name: 'string/ae_test', level: Level.info);
...
*/
    ...
    test('AE isValidVListLength VM.k1 good values', () {
      ...
      for (var i = 0; i < 10; i++) {
        ...
        for (var tag in aeTags0) {
          expect(AE.isValidVListLength(tag, vList), true);
    ...
    test('AE isValidVListLength VM.k1 bad values', () {
      for (var i = 1; i < 10; i++) {
        ...
        for (var tag in aeTags0) {
          global.throwOnError = false;
          expect(AE.isValidVListLength(tag, vList), false);

          expect(AE.isValidVListLength(tag, invalidVList), false);

          global.throwOnError = true;
          expect(() => AE.isValidVListLength(tag, vList),
              throwsA(const isInstanceOf<InvalidValuesError>()));
        }
      }
    });
    ...
    test('AE isValidVListLength VM.k1_n good values', () {
      ...
      for (var i = 1; i < 10; i++) {
        ...
        for (var tag in aeTags1) {
          log.debug('tag: $tag');
          expect(AE.isValidVListLength(tag, vList0), true);
