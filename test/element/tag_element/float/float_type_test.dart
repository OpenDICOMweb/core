...
void main() {
  ...
  group('Floating Point Values', () {
    test('Create Elements from floating values', () {
      ...
      final of0 =
          new OFtag(PTag.kUValueData, new Float32List.fromList(f32Values));
      expect(of0.values.first.toStringAsPrecision(1),
          equals((2047.99).toStringAsPrecision(1)));
      final od0 =
          new ODtag(PTag.kSelectorODValue, new Float64List.fromList(f32Values));
      expect(od0.values.first.toStringAsPrecision(1),
          equals((2047.99).toStringAsPrecision(1)));
    });

    //Urgent Sharath: Please add comment on goal of this test.
    ...
    //Urgent Sharath: not real test - no expect()
    test('update Elements from strings', () {
      ...
      final of0 = new OFtag(PTag.lookupByCode(kUValueData),
      ...
      final fd0 =
          new FDtag(PTag.kPhysicalDeltaX, new Float64List.fromList(fdValues))
            ..update(new Float64List.fromList(fdValues));
      log..debug(fd0.value)..debug(fd0.view());

      final od0 = new ODtag(PTag.lookupByCode(0x00720073),
