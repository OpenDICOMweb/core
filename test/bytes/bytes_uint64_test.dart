...
void main() {
  ...
  test('Basic Uint64 tests', () {
    final vList0 = rng.uint64List(5, 10);
    log.debug('vList0: $vList0');
    final bytes0 = Bytes.typedDataView(vList0);
    final vList1 = bytes0.asUint64List();
    expect(vList1, equals(vList0));
    log.debug('vList1: $vList1');
    final vList2 = bytes0.getUint64List();
    log.debug('vList2: $vList2');
    expect(vList2, equals(vList1));
    final vList3 = bytes0.asUint64List();
    log.debug('vList3: $vList3');
    expect(vList3, equals(vList0));
    expect(vList3, equals(vList2));
    final bytes1 = Uint64.toBytes(vList0);
    final vList4 = bytes1.asUint64List();
  ...
  test('Uint64 tests', () {
    ...
    final vList2 = bytes1.asUint64List();
    log.debug('vList2: $vList2');
  ...
  test('Uint64 asUint64List tests', () {
    const count = 10;
    for (var k = 0; k < count; k++) {
      final vList0 = rng.uint64List(k, count);
      ...
      for (var i = 0; i < vList0.length + 1; i++) {
        ...
        final vList4 = bytes0.asUint64List(0, vList0.length - i);
  ...
  test('Uint64 sublist tests', () {
    ...
    for (var k = 0; k < count; k++) {
      ...
      for (var i = 0; i < vList0.length + 1; i++) {
        ...
        final vList3 = bytes1.asUint64List();
        log.debug('vList3: $vList3');
        expect(vList3, equals(vList1));
        expect(vList3.buffer == bytes1.buffer, true);

        final vList4 = bytes2.asUint64List();
        log.debug('vList4: $vList4');
        expect(vList4, equals(vList2));
  ...
  test('Uint64 view tests', () {
    ...
    for (var k = 0; k < count; k++) {
      ...
      for (var i = 0; i < vList0.length + 1; i++) {
        ...
        final vList1 = Uint64List.view(vList0.buffer, j, vList0.length - i);
        log.debug('vList1: $vList1');
        expect(vList1.buffer == vList0.buffer, true);

        final vList2 = Uint64List.view(vList0.buffer, 0, vList0.length - i);
        log.debug('vList2: $vList2');
        expect(vList2.buffer == vList0.buffer, true);
