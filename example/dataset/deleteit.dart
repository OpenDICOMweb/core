//  Copyright (c) 2016, 2017, 2018,
//  Poplar Hill Informatics and the American College of Radiology
//  All rights reserved.
//  Use of this source code is governed by the open source license
//  that can be found in the odw/LICENSE file.
//  Primary Author: Jim Philbin <jfphilbin@gmail.edu>
//  See the AUTHORS file for other contributors.
//

import 'package:core/core.dart';

void main() {
  final ds = TagRootDataset.empty();
  //Map<int, Element> mappedElem = {};
  Tag tag = PTag.lookupByCode(kStudyDate); // Study Date
  final date = DAtag(tag, ['19990505']);
  ds[tag.code] = date;
  date
    ..checkValues(['3758'])
    ..checkValue('25240528');
//  final cpy = date.copy..values;
  print('copied values equals original values:');
  print(date.values.first);
  tag = PTag.lookupByCode(kSeriesDate); // Series Date
  ds[tag.code] = DAtag(tag, ['19990505']);
  tag = PTag.lookupByCode(kAcquisitionDate); // Acquisition Date
  ds[tag.code] = DAtag(tag, ['19990505']);
  tag = PTag.lookupByCode(kContentDate); // Image Date
  ds[tag.code] = DAtag(tag, ['19990505']);

  tag = PTag.lookupByCode(kStudyTime); // Study Time
  final time = TMtag(tag, ['105234.530000']);
  ds[tag.code] = time;

  tag = PTag.lookupByCode(kSeriesTime); // Series Time
  ds[tag.code] = TMtag(tag, ['105234.530000']);
  tag = PTag.lookupByCode(kAcquisitionTime); //   Acquisition Time
  ds[tag.code] = TMtag(tag, ['105234.530000']);
  tag = PTag.lookupByCode(kContentTime); // Image Time
  ds[tag.code] = TMtag(tag, ['105234.530000']);
  tag = PTag.lookupByCode(
      kMediaStorageSOPClassUID); // Media Storage SOP Class UID
  ds[tag.code] = UItag(tag, [kCTImageStorage]);
  tag = PTag.lookupByCode(
      kMediaStorageSOPInstanceUID); // Media Storage SOP Instance UID
  ds[tag.code] = UItag(
      tag, ['2.16.840.1.113662.2.1.4519.41582.4105152.419990505.410523251']);
  tag = PTag.lookupByCode(kTransferSyntaxUID); // Transfer Syntax UID
  ds[tag.code] = UItag(tag, [kExplicitVRLittleEndian]);
  tag = PTag.lookupByCode(kStudyInstanceUID); // Study Instance UID
  ds[tag.code] =
      UItag(tag, ['2.16.840.1.113662.2.1.1519.11582.1990505.1105152']);
  print(ds.studyUid);
  print(ds.transferSyntax);
}
